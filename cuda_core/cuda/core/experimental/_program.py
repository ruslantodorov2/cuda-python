...
class ProgramOptions:
    ...
    device_c: Optional[bool] = False  # /**< --device-c (-dc) Generate relocatable code that can be linked with other relocatable device code. Equivalent to --relocatable-device-code=true. */
    device_w: Optional[bool] = False  # /**< --device-w (-dw) Generate non-relocatable code. Equivalent to --relocatable-device-code=false. */
    relocatable_device_code: Optional[bool] = False  # /**< --relocatable-device-code={true|false} (-rdc) Enable (disable) the generation of relocatable device code. Default: false */
    extensible_whole_program: Optional[bool] = False  # /**< --extensible-whole-program (-ewp) Do extensible whole program compilation of device code. Default: false */
    device_debug: Optional[bool] = False  # /**< --device-debug (-G) Generate debug information. If --dopt is not specified, then turns off all optimizations. */
    generate_line_info: Optional[bool] = False  # /**< --generate-line-info (-lineinfo) Generate line-number information. */
    ...
    ftz: Optional[bool] = False  # /**< --ftz={true|false} (-ftz) When performing single-precision floating-point operations, flush denormal values to zero or preserve denormal values. Default: false */
    prec_sqrt: Optional[bool] = True  # /**< --prec-sqrt={true|false} (-prec-sqrt) For single-precision floating-point square root, use IEEE round-to-nearest mode or use a faster approximation. Default: true */
    prec_div: Optional[bool] = True  # /**< --prec-div={true|false} (-prec-div) For single-precision floating-point division and reciprocals, use IEEE round-to-nearest mode or use a faster approximation. Default: true */
    fmad: Optional[bool] = True  # /**< --fmad={true|false} (-fmad) Enables (disables) the contraction of floating-point multiplies and adds/subtracts into floating-point multiply-add operations. Default: true */
    use_fast_math: Optional[bool] = False  # /**< --use_fast_math (-use_fast_math) Make use of fast math operations. */
    extra_device_vectorization: Optional[bool] = False  # /**< --extra-device-vectorization (-extra-device-vectorization) Enables more aggressive device code vectorization in the NVVM optimizer. */
    modify_stack_limit: Optional[bool] = True  # /**< --modify-stack-limit={true|false} (-modify-stack-limit) On Linux, during compilation, use setrlimit() to increase stack size to maximum allowed. Default: true */
    dlink_time_opt: Optional[bool] = False  # /**< --dlink-time-opt (-dlto) Generate intermediate code for later link-time optimization. Implies -rdc=true. */
    gen_opt_lto: Optional[bool] = False  # /**< --gen-opt-lto (-gen-opt-lto) Run the optimizer passes before generating the LTO IR. */
    optix_ir: Optional[bool] = False  # /**< --optix-ir (-optix-ir) Generate OptiX IR. Only intended for consumption by OptiX through appropriate APIs. */
    jump_table_density: Optional[int] = 101  # /**< --jump-table-density=[0-101] (-jtd) Specify the case density percentage in switch statements. Default: 101 */
    device_stack_protector: Optional[bool] = False  # /**< --device-stack-protector={true|false} (-device-stack-protector) Enable (disable) the generation of stack canaries in device code. Default: false */
    define_macro: Optional[Union[str, Tuple[str, str]]] = None  # /**< --define-macro=<def> (-D) Predefine a macro. Can be either a string, inwhich case that macro will be set to 1, or a tuple of strings, in which case the first element is defined as the second*/
    ...
    no_source_include: Optional[bool] = False  # /**< --no-source-include (-no-source-include) Disable the default behavior of adding the directory of each input source to the include path. */
    std: Optional[str] = None  # /**< --std={c++03|c++11|c++14|c++17|c++20} (-std) Set language dialect to C++03, C++11, C++14, C++17 or C++20. Default: c++17 */
    builtin_move_forward: Optional[bool] = True  # /**< --builtin-move-forward={true|false} (-builtin-move-forward) Provide builtin definitions of std::move and std::forward. Default: true */
    builtin_initializer_list: Optional[bool] = True  # /**< --builtin-initializer-list={true|false} (-builtin-initializer-list) Provide builtin definitions of std::initializer_list class and member functions. Default: true */
    disable_warnings: Optional[bool] = False  # /**< --disable-warnings (-w) Inhibit all warning messages. */
    restrict: Optional[bool] = False  # /**< --restrict (-restrict) Programmer assertion that all kernel pointer parameters are restrict pointers. */
    device_as_default_execution_space: Optional[bool] = False  # /**< --device-as-default-execution-space (-default-device) Treat entities with no execution space annotation as __device__ entities. */
    device_int128: Optional[bool] = False  # /**< --device-int128 (-device-int128) Allow the __int128 type in device code. */
    ...
    display_error_number: Optional[bool] = True  # /**< --display-error-number (-err-no) Display diagnostic number for warning messages. Default: true */
    no_display_error_number: Optional[bool] = False  # /**< --no-display-error-number (-no-err-no) Disable the display of a diagnostic number for warning messages. */
    ...
    brief_diagnostics: Optional[bool] = False  # /**< --brief-diagnostics={true|false} (-brief-diag) Disable or enable showing source line and column info in a diagnostic. Default: false */
    ...
    fdevice_syntax_only: Optional[bool] = False  # /**< --fdevice-syntax-only (-fdevice-syntax-only) Ends device compilation after front-end syntax checking. */
    minimal: Optional[bool] = False  # /**< --minimal (-minimal) Omit certain language features to reduce compile time for small programs. */
    device_stack_protector: Optional[bool] = False  # /**< --device-stack-protector (-device-stack-protector) Enable stack canaries in device code. */

    def __post_init__(self):
        # Validate gpu_architecture
        valid_architectures = {
            "compute_50", "compute_52", "compute_53", "compute_60", "compute_61", "compute_62",
            "compute_70", "compute_72", "compute_75", "compute_80", "compute_87", "compute_89",
            "compute_90", "compute_90a", "sm_50", "sm_52", "sm_53", "sm_60", "sm_61", "sm_62",
            "sm_70", "sm_72", "sm_75", "sm_80", "sm_87", "sm_89", "sm_90", "sm_90a"
        }
        if self.gpu_architecture is not None and self.gpu_architecture not in valid_architectures:
            raise ValueError(f"Invalid gpu_architecture: {self.gpu_architecture}")

        # Validate jump_table_density
        if self.jump_table_density is not None and not (0 <= self.jump_table_density <= 101):
            raise ValueError(f"jump_table_density must be between 0 and 101, got {self.jump_table_density}")

        # Ensure mutually exclusive options are not both set
        if self.device_c and self.device_w:
            raise ValueError("device_c and device_w cannot both be True")
        ...
        if self.display_error_number:
            self.formatted_options.append("--display-error-number".encode())
        if self.no_display_error_number:
...
class Program:
    ...
    def compile(self, target_type, name_expressions=(), logs=None):
        ...
        if self._backend == "nvrtc":
            ...
            # TODO: allow tuples once NVIDIA/cuda-python#72 is resolved
            handle_return(
                nvrtc.nvrtcCompileProgram(self._handle, len(self.options), self.options),
                handle=self._handle)
